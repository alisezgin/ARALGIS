...
BOOL COdroidCommunicator::Start(SCNOTIFYPROC pNotifyProc, CMainFrame* pFrame)
{
	...
	if (!ThreadComm)
	{
		TRACE("_beginthreadex(...) failure, ThreadOdroidCommunicator::Start CommThread\n");
		return FALSE;
	}
					...
					else
					{
						...
						if (EventCaused == WAIT_OBJECT_0 + 1)
						{
							...
							else if (NetworkEvents.lNetworkEvents & FD_CLOSE)
							{
								...
								CMainFrame* pMFrame =
									static_cast<CMainFrame*>(pOdroidCommunicator->m_pFrame);

								pMFrame->PostMessage(WM_ODROID_LOST, 0, 0);
							} // FD_CLOSE
						} // network event
					}// client socket event
				} // while loop
...
UINT __stdcall COdroidCommunicator::CommThread(LPVOID pParam)
{
	...
	for (;;)
	{
		DWORD EventCaused = WaitForMultipleObjects(5,
			Handles,
			FALSE,
			INFINITE);
...
void COdroidCommunicator::SendOdroidOpenBarrierMessage()
{
	...
	messageToSend.messageFrom = 2;
	messageToSend.messageID = 4;
