...
Mat	get_presentable_color1(const int nColors){
...
map<float,int> get_map_of_absorption_to_daindex(const int nColors){
	map<float,int>	mapfi;
	Mat	tones1f	=	get_presentable_color1(nColors);
...
bool get_mode_param1(bool &is_swap,const int mode,int &m,int &n,float &a0,float &a1,const Mat &tones1f,const Mat &dst1f,const Mat &init1f,const int &i,const int &j,float &new_cur_v,float &new_nei_v){
...
double calc_distance(const Mat &src1f){
	...
	if(distance>=0.5){
		return (float) (1. / sqrtf(1. - distance));
	}else{
		return (float)(1. / sqrtf(distance));
	}
}
		...
		for(int i=0;i<height;i++){	// entire image
			for(int j=0;j<width;j++){
				...
				for(int mode=0;mode<nModes;mode++){
					...
					get_mode_param1(is_swap,mode,m,n,a0,a1,tones,dst1f,init1f,i,j,new_cur_v,new_nei_v);	// set position
				...
				if(tempMindE<0.){	// error is reduced
					...
					bool is_swap=false;
					float	new_cur_v,new_nei_v;
					get_mode_param1(is_swap,tempMinNumber,nm,nn,a0,a1,tones,dst1f,init1f,i,j,new_cur_v,new_nei_v);
...
bool pixkit::multitoning::ordereddithering::ChanduStanichWuTrager2014_genDitherArray(std::vector<cv::Mat> &vec_DA1b, int daSize, int nColors,float D_min){
	...
	//////////////////////////////////////////////////////////////////////////
	// tones
	Mat	tones	=	get_presentable_color1(nColors);
...
bool pixkit::multitoning::ordereddithering::ChanduStanichWuTrager2014(const cv::Mat &src1b, const std::vector<cv::Mat> &vec_DA1b,cv::Mat &dst1b){	
	...
	Mat	tones1f	=	get_presentable_color1(nColors);	// from range of 0 to 1
