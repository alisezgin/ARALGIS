...
bool SimplestColorBalance1(cv::Mat ori,float upperthresh,float lowerthresh){
...
// boraN begin
//bool pixkit::enhancement_local::MSRCP2014(const cv::Mat &src,cv::Mat &dst){
bool pixkit::enhancement_local::MSRCP2014(const cv::Mat &src, cv::Mat &dst, int Scl1, int Scl2, int Scl3, float lPer, float uPer){
	...
	int RetinexScales[3] = { Scl1, Scl2, Scl3 };
	// boraN end
	...
	*/
	...
	//SimplestColorBalance1, see IPOL for more information
	cv::normalize(RetinexOut,RetinexOut,0,255,32);
	// boraN begin
	//SimplestColorBalance1(RetinexOut,0.01,0.01);
	SimplestColorBalance1(RetinexOut, lPer, uPer);
	// boraN end
