...
bool pixkit::enhancement_local::LCE_BSESCS2014(const cv::Mat &src,cv::Mat &dst,cv::Size blockSize){
...
bool pixkit::enhancement_local::Lal2014(const cv::Mat &src,cv::Mat &dst, cv::Size title, float L,float K1 ,float K2 ){
...
bool pixkit::enhancement_local::Kimori2013(const cv::Mat &src,cv::Mat &dst,const cv::Size B, const int N){
...
bool pixkit::enhancement_local::LiWangGeng2011(const cv::Mat & ori,cv::Mat &ret){
...
bool pixkit::enhancement_local::Sundarami2011(const cv::Mat &src,cv::Mat &dst, cv::Size N, float L, float phi){
...
bool pixkit::enhancement_local::LiuJinChenLiuLi2011(const cv::Mat &src,cv::Mat &dst,const cv::Size N){
...
bool pixkit::enhancement_local::LambertiMontrucchioSanna2006(const cv::Mat &src,cv::Mat &dst,const cv::Size B,const cv::Size S){
	...
	//////////////////////////////////////////////////////////////////////////
	// exception
	if(blockSize.height>src.rows||blockSize.width>src.cols||blockSize.height==1||blockSize.width==1)
	{	// block size should be even (S3P5-Step.2)
		return false;
	}
...
bool pixkit::enhancement_local::FAHE2006(const cv::Mat &src1b,cv::Mat &dst1b,cv::Size blockSize){
	...
	if(src1b.type()!=CV_8UC1){
		CV_Error(CV_StsBadArg,"[pixkit::enhancement_local::FAHE2006] allows only grayscale image.");
	}
...
bool pixkit::enhancement_local::YuBajaj2004(const cv::Mat &src,cv::Mat &dst,const int blockheight,const int blockwidth,const float C,bool anisotropicMode,const float R){
...
bool pixkit::enhancement_local::KimKimHwang2001(const cv::Mat &src,cv::Mat &dst,const cv::Size B,const cv::Size S){
...
bool pixkit::enhancement_local::Stark2000(const cv::Mat &src,cv::Mat &dst,const int blockheight,const int blockwidth,const float alpha,const float beta){
...
bool pixkit::enhancement_local::CLAHEnon1987(const cv::Mat &src,cv::Mat &dst, cv::Size nBlock, float L ){
	///////////////////////////////////////////////////////////////////////////////////////////////////
	if(src.type()!=CV_8UC1){
		return false;
	}
...
bool pixkit::enhancement_local::CLAHE1987(const cv::Mat &src,cv::Mat &dst, cv::Size blockSize, float L){
	///////////////////////////////////////////////////////////////////////////////////////////////////
	if(src.type()!=CV_8UC1){
		return false;
	}
...
bool pixkit::enhancement_local::AHE1974(const cv::Mat &src1b,cv::Mat &dst1b,const cv::Size blockSize){
	...
	if(src1b.type()!=CV_8UC1){
		CV_Error(CV_StsBadArg,"[pixkit::enhancement_local::AHE1974] allows only grayscale image.");
	}
...
/// boraN begin
//bool pixkit::enhancement_global::RajuNair2014(const cv::Mat &src,cv::Mat &dst){
bool	pixkit::enhancement_global::RajuNair2014(const cv::Mat &src, cv::Mat &dst, float K, float E){
...
bool pixkit::enhancement_global::LeeLeeKim2013(const cv::Mat ori,cv::Mat &ret,double alpha){
...
bool pixkit::enhancement_global::MaryKim2008(const cv::Mat &src, cv::Mat &dst,int MorD , int r){
...
///////////////////////////////
void drawHistogram(int b_hist[], int XX)
{
#define MAX_HIST 256
	...
	char title[1000];
	char sNum[20];
	_itoa_s(XX, sNum, sizeof(sNum), 10);
	strcpy_s(title, sNum);
...
////////////////////////////////////
bool pixkit::enhancement_global::WadudKabirDewanChae2007(const cv::Mat &src, cv::Mat &dst, const int x){
	...
	for(int i=0;i<256;i++)
	{
		...
		for (int j=-1;j<2;j++)
		{
			if ((i+j)>=0 && (i+j)<256)
			{
				sum1+=(float)hist[i+j]*(float)smoothfilter[j+1];
				tempnum++;
			}	
		}
		tempnum*=smoothfilter[0];
		hist[i]=(int)(sum1/(float)tempnum+0.5);
	}

	/////////////////////////////
	//drawHistogram(hist, 1);
	////////////////////////////////
	...
	for (int i=0;i<256;i++)
	{
		...
		while (PartitionFlag)
		{
			...
			{
				mean /= sum;
				//mean = sum / (minima[count - 1] - minima[count - 2]);
			}
			else
			{
				mean = 0;
			...
			for (int k=minima[count-2];k<=minima[count-1];k++)
			{
				...
				//sd += ((double)hist[k] - mean) * ((double)hist[k] - mean);
			}
			// boraN begin
			//sd=sqrt(sd/sum);
			if (sum > 0.0)
			{
				sd = sqrt(sd / sum);
				//sd = sqrt (sd / (minima[count - 1] - minima[count - 2]));
			}
			else
			{
				sd = 0;
			...
			{						//	low-minima �� (mean-sd)�����������P�w.
				...
				SubHistFlag=true;
				if (minima[count] > 255 || minima[count-1] > 255)
				{
					//AfxMessageBox(_T(L"XXXX in WadudKabirDewanChae2007"));
				}
				aa++;
			}
		}
...
bool pixkit::enhancement_global::GlobalHistogramEqualization1992(const cv::Mat &src,cv::Mat &dst){

	if(src.type()!=CV_8UC1){
		CV_Assert(CV_StsUnmatchedFormats,"[pixkit::enhancement_global::GlobalHistogramEqualization1992] src should be CV_8UC1.");
	}
