...
bool EM_Alg(cv::Mat src,std::vector<float> &Mean, std::vector<float> &Sd, std::vector<float> &W){
	...
	const int K = (int) Mean.size();  //高斯模型的數目
...
bool pixkit::enhancement_global::CelikTjahjadi2012(cv::Mat &src,cv::Mat &dst,int N){
	...
	//給予初值
	for(int i=0;i<N;i++){
		Mean[i] = (float) (nColors / N*i + nColors / N / 2);
		Sd[i] = 8;
		W[i] = (float) (1.0 / N);
	}
	...
	for(int i=0;i<nColors;i++)
		for(int k=0;k<K;k++){
			cdf[i] += (float)(W[k] * calcCDF((double)i, Mean[k], Sd[k]));
		}
