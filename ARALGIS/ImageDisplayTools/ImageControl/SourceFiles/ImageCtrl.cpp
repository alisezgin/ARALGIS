...
#include "..\\HeaderFiles\\ImageCtrl.h"
...
BOOL CImageCtrl::load(cv::Mat& aInMat)
{
	int dHeight = aInMat.cols;
	int dWidth = aInMat.rows;
	...
	for (int i = 0; i < dWidth; i++)
	{
		...
		{
			dst = m_outImage->GetPixelAddress(i, j);
			src = pTemp;
...
void CImageCtrl::OnLButtonUp(UINT nFlags, CPoint point)
{
	if (m_isImageShown && m_isPanMode) 
	{ 
		m_panOffset = point - m_panAtPt; m_panAtPt = point; 
		Invalidate(); 
		UpdateWindow(); 
	} 
	ReleaseCapture();
...
BOOL CImageCtrl::bMouseWheel(UINT nFlags, short zDelta, CPoint pt)
{
	if (m_isImageShown && m_isZoomMode)
	{
		m_isInitialShow = FALSE;
