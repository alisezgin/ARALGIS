#include "stdafx.h"
#include "..\\HeaderFiles\\Icon.h"
#include "..\\HeaderFiles\\ImageControl.h"
...
void CImageControl::release(bool calcFrameAspectRatio)
{
	CRect rect;
...
BOOL CImageControl::load(CString szFilePath)
{
	release(false);
	if ((m_pBmp = Bitmap::FromFile(szFilePath)) && m_pBmp->GetLastStatus() != Status::Ok) 
	{ 
		delete m_pBmp; 
		m_pBmp = NULL; 
	}
...
BOOL CImageControl::load(IStream* piStream)
{
	release(true);
	if ((m_pBmp = Bitmap::FromStream(piStream)) && m_pBmp->GetLastStatus() != Status::Ok) 
	{ 
		delete m_pBmp; 
		m_pBmp = NULL; 
	}
...
BOOL CImageControl::load(BYTE* pData, size_t nSize)
{
	...
	if (CreateStreamOnHGlobal(NULL, TRUE, &pStream) == S_OK)
	{
		if (pStream->Write(pData, (ULONG)nSize, NULL) == S_OK)
			if ((m_pBmp = Bitmap::FromStream(pStream)) && m_pBmp->GetLastStatus() != Status::Ok) 
			{ 
				delete m_pBmp; 
				m_pBmp = NULL; 
			}
...
BOOL CImageControl::load(cv::Mat& aInMat)
{
	int dHeight = aInMat.rows;
	int dWidth = aInMat.cols;
	int dBytePP = aInMat.channels();

	release(false);

	CImage *m_outImage = NULL;
	m_outImage = new CImage;

	m_outImage->Create(dWidth, -dHeight, dBytePP * 8, 0);
	...
	if (dBytePP == 1)
	{
		...
		m_outImage->SetColorTable(0, 256, tab);
		delete[] tab;
	}

	int dByteImageSize = (unsigned int)((int)aInMat.step[0] * (int)aInMat.rows);

	BYTE* dByteImage = new BYTE[dByteImageSize];

	memcpy(dByteImage, aInMat.data, dByteImageSize);

	BYTE* pTemp;
	void* dst;
	const void* src;

	pTemp = (BYTE*)dByteImage;

	for (int i = 0; i < dHeight; i++)
	{
		for (int j = 0; j < dWidth; j++)
		{
			dst = m_outImage->GetPixelAddress(j, i);
			src = pTemp;
			memcpy(dst, src, dBytePP);
			pTemp = pTemp + dBytePP;
		}
	}

	//m_outImage->Save(L"C:\\Users\\bora\\Desktop\\sil\\1.jpg", Gdiplus::ImageFormatJPEG);

	m_pBmp = new Bitmap(m_outImage->Detach(), NULL);
...
BOOL CImageControl::load2(cv::Mat& aInMat)
{
	int dHeight = aInMat.rows;
	int dWidth = aInMat.cols;
	int dBytePP = aInMat.channels();
...
BOOL CImageControl::load(HMODULE hModule, LPCTSTR lpName, LPCTSTR lpType, WORD wlan)
{
	HRSRC hRes; 
	DWORD resSize; 
	HGLOBAL hGlobal, hGlobal2, hGlobal3; 
	void *pRes = NULL, *pRes2 = NULL; IStream *pStream = NULL; Bitmap *pBmp = NULL; HICON hIcon; GRPICONDIR *pIconDir = NULL;
		 ...
		 else // https://msdn.microsoft.com/en-us/library/ms997538.aspx
		 {
			 if (pIconDir = (GRPICONDIR*)LockResource(hGlobal))
			 {
				 if (pIconDir->idCount && 
					 ...
					 (hGlobal3 = LoadResource(hModule, hRes)))
				 {
...
BOOL CImageControl::convert(CString pathName, CString imageType)
{
	BOOL ok = FALSE; 
	UINT j, n, size; 
	CString fd; 
						...
						if(graphics.DrawImage(m_pBmp, 0, 0, (int)(m_width + 1e-6), (int)(m_height + 1e-6)) == Status::Ok)
							if (CreateStreamOnHGlobal(0, TRUE, &pStream) == S_OK)
							{
								if (pBmp->Save(pStream, &pImageCodecInfo[j].Clsid) == Status::Ok)
									if ((GetHGlobalFromStream(pStream, &hGlobal) == S_OK) && (N = (DWORD)GlobalSize(hGlobal)) && (pBuffer = GlobalLock(hGlobal)))
									{
...
BOOL CImageControl::iconResourceToFile(CString resPathName, LPCTSTR lpName, WORD wlan, CString icoPathName)
{
	BOOL ok = FALSE; 
	HMODULE hModule; 
	CIconExtractor ie;
  
	if (hModule = LoadLibrary(resPathName)) 
	{ 
		ok = (ie.importIcon(hModule, lpName, wlan) && ie.exportIcon(icoPathName)); 
		FreeLibrary(hModule); 
	}
...
void CImageControl::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	...
	if (pDC = GetDC())
	{
		if (m_pBmp)
		{
			...
			if (m_isInitialShow)
			{
				...
				else
				{
					if (m_sizeType == sizeType::SIZE_SCALETOFIT)
					{
						...
						s = (sx > sy) ? sy : sx; 
						m_width = s * m_widthOriginal;
						m_height = m_aspectRatio * m_width;
					}
			...
			else if (m_zoomAtPt.x < 0) // PAN action.
			{ 
			...
			else if (m_zoomFactor > 1e-6)  // ZOOM action.                         
			{
				...
				if (w0 >= m_zoomMin && w0 <= m_zoomMax && h0 >= m_zoomMin && h0 <= m_zoomMax) // "ZOOM OUT" rectangle can't have a side smaller than m_ZoomMin pixels, 
					                                                                          // "ZOOM iN" can't have a side greater than m_zoomMax pixels.
				{
					dx *= (w0 / m_width); 
					dy *= (h0 / m_height);
					m_left = m_zoomAtPt.x - dx; 
					m_top = m_zoomAtPt.y - dy; 
					m_width = w0;
					m_height = m_aspectRatio * m_width;
...
void CImageControl::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	{ 
		...
		if (dParent != NULL)
		{
			dParent->SendMessage(WM_LMOUSE_DOWN, nFlags, 0);
		}

		//CFrameWnd* dFrm;
		//dFrm = GetTopLevelFrame();
		//CView* pView = dFrm->GetActiveView();
		//pView->SendMessage(WM_LMOUSE_DOWN, nFlags, 0);
	}
...
void CImageControl::OnLButtonUp(UINT nFlags, CPoint point)
{
	...
	{ 		
		...
		if (dParent != NULL)
		{
			dParent->SendMessage(WM_LMOUSE_UP, nFlags, 0);
		}

		//CFrameWnd* dFrm;
		//dFrm = GetTopLevelFrame();
		//CView* pView = dFrm->GetActiveView();
		//pView->SendMessage(WM_LMOUSE_UP, nFlags, 0);
		//Invalidate(); 
		//UpdateWindow(); 
	} 
	ReleaseCapture();
...
void CImageControl::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	{ 
		...
		if (dParent != NULL)
		{
			dParent->SendMessage(WM_LMOUSE_MOVE, nFlags, 0);
		}

		//CFrameWnd* dFrm;
		//dFrm = GetTopLevelFrame();
		//CView* pView = dFrm->GetActiveView();
		//pView->SendMessage(WM_LMOUSE_MOVE, nFlags, 0);
		//Invalidate(); 
		//UpdateWindow();
	}
}
...
BOOL CImageControl::OnMouseWheel(UINT nFlags, short zDelta, CPoint pt)
{
	...
	//	m_isInitialShow = FALSE; 
	//	m_zoomAtPt = pt; 
	//	m_zoomFactor = 1 + (zDelta / 1200.0); 

	//	SetPoint(pt);
	//	CFrameWnd* dFrm;
	//	dFrm = GetTopLevelFrame();
	//	CView* pView = dFrm->GetActiveView();
	//	pView->SendMessage(WM_MOUSE_WHEEL, nFlags, zDelta);
...
BOOL CImageControl::bMouseWheel(UINT nFlags, short zDelta, CPoint pt)
{
	if (m_isImageShown && m_isZoomMode)
	{
		m_isInitialShow = FALSE;
		m_zoomAtPt = pt;
		m_zoomFactor = 1 + (zDelta / (120.0*3.0));
		Invalidate();
		UpdateWindow();
		return TRUE;
	}
...
void CImageControl::SetPoint(CPoint aInPoint)
{
	m_MousePoint = aInPoint;
}
CPoint CImageControl::GetPoint()
{
	return m_MousePoint;
}

void CImageControl::OnLButtonDblClk(UINT nFlags, CPoint point)
{
