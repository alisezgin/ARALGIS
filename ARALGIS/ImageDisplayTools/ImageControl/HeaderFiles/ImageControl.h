...
public:
	...
	BOOL load(cv::Mat& aInMat);                                                        // Loads an image from opencv matrix object.
	BOOL load2(cv::Mat& aInMat);                                                        // Loads an image from opencv matrix object.
	BOOL bMouseWheel(UINT nFlags, short zDelta, CPoint pt);
	void SetPoint(CPoint aInPoint);
	CPoint GetPoint();
